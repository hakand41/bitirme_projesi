...
def generate_and_save_comparison_json(app_context, content_id, file1_info, file2_info, json_results_folder):
    ...
    with app_context: # Flask uygulama bağlamını aktif et
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT DetailId FROM ComparisonJsonDetail 
                WHERE ContentId = ? AND 
            ...
            if existing_detail:
                ...
                cursor.execute("""
                    INSERT INTO ComparisonJsonDetail (ContentId, FirstFileId, SecondFileId, JsonFilePath, CreatedAt)
                    VALUES (?, ?, ?, ?, ?)
